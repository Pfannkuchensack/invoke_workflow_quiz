/**
 * Types for Workflow Quiz Game Backend
 * (Copied from shared types to avoid rootDir issues)
 */
...
// Player node info for validation (maps player-node IDs to their types)
export interface PlayerNodeMapping {
  id: string;
  nodeType: string;
}
...
export interface ValidateQuizRequest {
  ...
  playerNodeMappings?: PlayerNodeMapping[]; // Maps player-node IDs to their types
}
